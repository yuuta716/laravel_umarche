                                        ...
                                        @foreach ($categories as $category)
                                            ...
                                            <optgroup label="{{ $category->name }}">
                                                ...
                                                @foreach ($category->secondary as $secondary)
                                                    <option value="{{ $secondary->id }}">{{ $secondary->name } @if($secondary->id === $product->secondary_category_id) selected @endif}
                                                    </option>
                                                @endforeach
                                            </optgroup>
                                        @endforeach
...
//                         {{-- curentId="// {{$product->image1}}"で数字が⼊り currentImage="{{$product->imageFirst->filename ?? ''}}"の⽅で⽂字
                        // 列が⼊りnullなら''空に--}}
                        ...
                        <div class="p-2 w-1/2 mx-auto">
                            <div class="relative flex justify-around">
                                ...
                                <div><input type="radio" name="is_selling" value="0" class="mr-2" @if ($product->is_selling === 0) {checked} @endif>停⽌中</div>
                            </div>
                        </div>
    ...
    rounded text-lg">戻る</button>
                            <button type="submit" class=" mx-auto text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600
    rounded text-lg">更新する</button>
                        </div>
                </div>
                </form>
            </div>
